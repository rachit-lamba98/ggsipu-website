...
<?php
	for($i=date('m');$i!=0;$i--)
	{
		...
		$sql = "SELECT * FROM ".$table_name." WHERE MONTH(uploading_date) = $i AND YEAR(uploading_date) = ".date('Y')." AND s_no < (SELECT MAX(s_no) FROM ".$table_name.")-9 ORDER BY s_no desc" ;
		$result = mysqli_query( $conn,$sql );
...
<?php
            $sql = "SELECT * FROM ".$table_name." WHERE MONTH(uploading_date) = $j AND YEAR(uploading_date) = ".$year." ORDER BY s_no" ;
            $result = mysqli_query($conn, $sql);
